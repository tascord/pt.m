...
import { CityLoop, resolve_colour } from "~/databank/Stations";

const route = useRoute();
const { pending, data: line, } = useAsyncData('lines', () => $fetch(`/api/line/${route.params.id}`));
const direction = ref('to_city');

const priority_string = (code) => {

    const priority = Priority.from(code);
    let priorities = [];
        ...
        <div class="child-h1:text-3xl child-h1:text-bold">
            ...
            <div class="mt-1 opacity-70">
                <h2>
                    ...
                    servicing the
                    <NuxtLink v-for="line in line.stops[0].services.lines" :to="`/line/${line}`">
                        {{ line }}
                    </NuxtLink>
            ...
            <div class="flex-1">
                <div>
                    ...
                    <div class="
                        space-x-4
                        text-xs
                        child:rounded-md child:shadow-sm
                        child:transition-all child:duration-200 child:ease-in-out
                        child-button:p-2
                    ">
                        <button v-on:click="() => direction = 'to_city'"
                            :class="(direction !== 'to_city' ? 'text-opacity-40' : 'bg-purple-500 text-opacity-100 px-2') + '  py-[0.15rem]'">
                            <Icon name="tabler:building-skyscraper" /> To the City
                        </button>
                        <button v-on:click="() => direction = 'to_suburbs'"
                            :class="(direction !== 'to_suburbs' ? 'text-opacity-40' : 'bg-purple-500 text-opacity-100 px-2') + ' py-[0.15rem]'">
                    ...
                    <ul class="mt-2">
                        <li v-for="stop in (direction === 'to_suburbs' ? line.stops : [...line.stops].reverse())"
                            :key="stop.id" class="flex text-lg ">
